<#
.SYNOPSIS
    ROMAN ADAPTIVE STORAGE ENGINE (RASE) v73.6.9 Single-Console Safety Edition.
    Intelligent, adaptive PowerShell monolith for deep storage diagnostics and OS optimization.

.DESCRIPTION
    RASE v73.6.9 is an Enterprise-grade maintenance framework. 
...
.NOTES
    v73.6.9 - transcript hygiene in the cleanup engine. No functional change.
      ...
      - Windows Update cache and temp-file removals use -ErrorAction SilentlyContinue and then
        verify with Test-Path, instead of -ErrorAction Stop inside a try/catch. A file held open
        by Windows Update or by a running process is an expected outcome of these operations,
        but under $ErrorActionPreference = "Stop" each one became a terminating error, and
        Start-Transcript recorded "PS>TerminatingError(Remove-Item)" in the protocol even though
        the catch handled it. A correctly accounted condition read like a crash to anyone
        opening the transcript.
      - Accounting is unchanged in intent and slightly stronger in fact: a removal now counts as
        successful only when the item is verifiably gone, rather than when no exception was
        raised. A partially deleted directory still exists and is still counted as failed.
      - The temp-file loop had the same pattern and was fixed with it. It never fired on the
        acceptance machine only because every matched file happened to be unlocked; on a busier
        system it could have produced dozens of those lines.
      - Temp directory removals keep their previous accounting exactly, including the fact that
        a directory which cannot be removed is not counted as a failure.
      - Nothing else touched: Defrag, NTFS, VSS, WU status logic, scoring, disk detection,
        Restore Point, single-console elevation, reboot handling and the phase pipeline are all
        unchanged.

    v73.6.8 - corrections driven by a direct probe of the target machine rather than by
    inference. The observed output contradicted two assumptions carried since v73.6.5.
      - "fsutil behavior query disable8dot3" prints a fully localized sentence with NO English
        token on a localized build - not the registry value name, not "disable8dot3". The
        English-anchor tiers can never resolve 8.3 there, so a separator-anchored parse is
        restored as a third tier. It matches only a number immediately following '=' or ':'
        (punctuation is not translated) and takes the first such number, because a scan for
        "any digit" would return the 8 or the 3 out of the word "8dot3".
      - NtfsDisableLastAccessUpdate does not use the documented 0-3 encoding. The probe read
        0x80000001 on a volume fsutil describes as system-managed with updates disabled, which
        is state 3. Bit 31 is the system-managed flag and bit 0 is the disable bit, so the
        previous "-band 0x3" returned 1 and silently reported the volume as user-managed.
        Decoding is now per-behavior: disable8dot3 keeps the plain 0-3 encoding, where the same
        transform would have turned "per volume policy" (2) into 0.
      - Changelog rebuilt into a linear sequence; the invented "v73.6.6-a" label is gone and the
        Single-Console work is described inside the v73.6.6 entry it actually shipped with.

    v73.6.7 - re-merge plus two findings from the first Full acceptance runs.
      - Re-applies v73.6.5, which was lost: v73.6.6 was branched from v73.6.4, so the per-
        behavior fsutil anchor list, the system-managed high-bit handling and the fsutil
        diagnostic echo were silently reverted. Nothing in v73.6.6 replaced them.
      - fsutil is now called with $ErrorActionPreference temporarily set to Continue. In
        Windows PowerShell 5.1, "2>&1" on a native command while the preference is Stop turns
        any stderr output into a terminating NativeCommandError, which aborts the function
        before the output can be parsed or even recorded - the most likely reason the v73.6.5
        diagnostic line never appeared in the report.
      - Fragmentation analysis moved to Win32_Volume.DefragAnalysis. Optimize-Volume -Analyze
        writes its report to the verbose stream and returns nothing on the output stream, so
        $analysis.PercentFragmentation was always $null and the defrag path took the "no usable
        data" branch on every disk on every machine. RASE refused to defragment safely rather
        than wrongly, but the feature had never actually run. Optimize-Volume is kept as a
        secondary attempt.
      - Stop-Transcript now runs before the "press Enter to close" wait. Until it does, the
        transcript has no footer; both Full acceptance transcripts arrived truncated because of
        it, and a console closed with the X button would leave the file incomplete for good.
      - Changelog corrected: v73.6.6 was used twice as a heading (the WU accounting fix and the
        Single-Console edition), and v73.6.5 had no entry at all.

    v73.6.6 - Windows Update cleanup accounting fix and Single-Console Safety Edition.
    ...
    v73.6.5 - fsutil detection fix. Lost when v73.6.6 branched from v73.6.4; re-applied in
    v73.6.7 and corrected against real output in v73.6.8.
      - Each fsutil behavior carries an ordered list of English anchor tokens.
      - ConvertTo-RaseFsBehaviorValue handles the system-managed bit that a plain [int]
...
#>
# ============================================================
# ROMAN ADAPTIVE STORAGE ENGINE (RASE) v73.6.9 MONOLITH
# ============================================================
            ...
            try {
                ...
                if (Test-Path $wuPath) {
                    foreach ($item in @(Get-ChildItem -Path $wuPath -Force -ErrorAction SilentlyContinue)) {
                        $size = 0
                        try {
                            $size = if ($item.PSIsContainer) { (Get-ChildItem -LiteralPath $item.FullName -Recurse -Force -ErrorAction SilentlyContinue | Measure-Object -Property Length -Sum).Sum } else { $item.Length }
                        }
                        catch { $size = 0 }

                        # SilentlyContinue rather than Stop, and then verify. A locked manifest in
                        # SoftwareDistribution is an EXPECTED outcome of this operation, not an
                        # exception: Windows Update holds those files, and RASE already reports the
                        # count correctly. With -ErrorAction Stop, $ErrorActionPreference turns each
                        # one into a terminating error, and Start-Transcript writes
                        # "PS>TerminatingError(Remove-Item): ... access denied" into the protocol
                        # even though the catch handled it - so a normal, correctly accounted
                        # condition reads like a crash to whoever opens the transcript afterwards.
                        #
                        # The accounting is not weakened by this. Success is now confirmed by
                        # checking that the item is actually gone, which is a stronger claim than
                        # "no exception was raised": a directory that was only partially deleted
                        # still exists and is still counted as a failure.
            ...
            try {
                ...
                foreach ($path in $tempPaths) {
                    if (Test-Path $path) {
                        ...
                        # Same treatment as the Windows Update cache loop above, and for the same
                        # reason: a temp file held open by a running process is an expected outcome,
                        # not an exception. It never fired on the acceptance machine because every
                        # matched file happened to be free - on a busier system this loop could have
                        # filled the transcript with dozens of TerminatingError lines for a condition
                        # RASE handles correctly. Removal is verified rather than assumed.
                        foreach ($f in ($matched | Where-Object { -not $_.PSIsContainer })) {
                            ...
                            if (Test-Path -LiteralPath $f.FullName) {
                                ...
                                $deleted++
                                if ($fileSize) { $freedBytes += $fileSize }
                            }
                        }
                        ...
                        foreach ($d in ($matched | Where-Object { $_.PSIsContainer } | Sort-Object { $_.FullName.Length } -Descending)) {
                            ...
                            if ($remaining.Count -eq 0) {
                                ...
                                # Counted exactly as before: a directory that could not be removed is
                                # not treated as a failure here, because the previous catch block
                                # swallowed that case too. Only the transcript noise is gone.
                                if (-not (Test-Path -LiteralPath $d.FullName)) { $deleted++ }
                            }
                        }
                    }
                }
...
# Returns the fragmentation percentage for a drive letter, or $null when it cannot be
# determined. Win32_Volume's DefragAnalysis method returns a structured Win32_DefragAnalysis
# instance (FilePercentFragmentation / TotalPercentFragmentation), so no output text is parsed
# and no display language is involved.
function Get-RaseFragmentationPercent {
    param([Parameter(Mandatory)][string]$DriveLetter)

    try {
        $dl = $DriveLetter.TrimEnd(':')
        $volume = Get-CimInstance -ClassName Win32_Volume -Filter "DriveLetter='${dl}:'" -ErrorAction Stop | Select-Object -First 1
        if ($null -eq $volume) { return $null }

        $result = Invoke-CimMethod -InputObject $volume -MethodName DefragAnalysis -ErrorAction Stop
        if ($null -eq $result) { return $null }

        $rvProp = $result.PSObject.Properties['ReturnValue']
        if ($rvProp -and $null -ne $rvProp.Value -and [int]$rvProp.Value -ne 0) { return $null }

        $analysisProp = $result.PSObject.Properties['DefragAnalysis']
        if (-not $analysisProp -or $null -eq $analysisProp.Value) { return $null }
        $analysis = $analysisProp.Value

        # FilePercentFragmentation is what defrag.exe reports as "fragmented files"; fall back to
        # the total figure when the file-level counter is not populated.
        foreach ($name in @('FilePercentFragmentation', 'TotalPercentFragmentation')) {
            $prop = $analysis.PSObject.Properties[$name]
            if ($prop -and $null -ne $prop.Value) { return [int]$prop.Value }
        }
        return $null
    }
    catch {
        LogError -Message "Fragmentation analysis via Win32_Volume failed for ${DriveLetter}:." -Source "Defrag" -Ex $_.Exception
        return $null
    }
}
...
function Invoke-Defrag {
    ...
    foreach ($dl in $diskTask.DriveLetters) {
        ...
        if (-not (Test-RaseDryRun "Analyze fragmentation and, if needed, defragment drive ${dl}:")) {
            try {
                # Win32_Volume.DefragAnalysis is the primary source. Optimize-Volume -Analyze
                # writes its report to the VERBOSE stream and returns nothing on the output
                # stream, so $analysis.PercentFragmentation was always $null and this function
                # took the "no usable data" branch on every disk, on every machine - which is
                # exactly what the first Full acceptance run showed for G:. The CIM method
                # returns a structured object instead, and its numbers are locale-independent.
                $frag = Get-RaseFragmentationPercent -DriveLetter $dl

                if ($null -eq $frag) {
                    # Secondary attempt, kept for systems where the CIM method is unavailable.
...
# of the NTFS module.
# Per-behavior detection metadata. Every field below is set from observed output rather than
# from assumption - a probe on Windows 10 LTSC 2019 (build 17763, Russian display language)
# produced this, and it is not what the earlier guesses predicted:
#
#   fsutil behavior query disablelastaccess
#       DisableLastAccess = 1  (<localized description of the managed state>)
#   fsutil behavior query disable8dot3
#       <fully localized sentence>: 1 (<localized description>).
#
# So the SHORT English token appears for lastaccess, and disable8dot3 carries NO English token
# at all on a localized build - not the registry value name, not "disable8dot3". Anchoring on
# English words alone can therefore never resolve 8.3 on such a system; only the registry or a
# separator-anchored parse can.
#
#   RegistryName          - the value fsutil itself reads and writes. Authoritative, locale-free.
#   Anchors               - English tokens to try in fsutil output, longest first. May be empty.
#   HighBitIsSystemManaged - true when the registry encodes "system managed" in bit 31 instead of
#                            in the documented bit 1.
$Global:RaseFsBehaviorInfo = @{
    "disable8dot3"      = @{
        RegistryName           = "NtfsDisable8dot3NameCreation"
        Anchors                = @("NtfsDisable8dot3NameCreation", "Disable8dot3")
        HighBitIsSystemManaged = $false
    }
    "disablelastaccess" = @{
        RegistryName           = "NtfsDisableLastAccessUpdate"
        Anchors                = @("NtfsDisableLastAccessUpdate", "DisableLastAccess")
        HighBitIsSystemManaged = $true
    }
}
$Global:RaseFileSystemRegKey = "HKLM:\SYSTEM\CurrentControlSet\Control\FileSystem"

# Diagnostics from the most recent fsutil query, so the report can show what it actually
# received when detection fails instead of only saying that it could not tell.
$Global:RaseLastFsutilOutput = $null

function ConvertTo-RaseFsBehaviorValue {
    # Normalises a raw setting value into the documented 0-3 state.
    #
    # NtfsDisableLastAccessUpdate does not store the documented encoding. The probe read
    # 0x80000001 on a volume that fsutil describes as system-managed with updates disabled -
    # that is state 3, but a plain "-band 0x3" yields 1 ("user managed, disabled") and quietly
    # loses the management mode. Bit 31 is the system-managed flag; bit 0 is the disable bit.
    # So the documented state is (bit 0) combined with 2 when bit 31 is set.
    #
    # disable8dot3 uses the plain documented encoding, where 2 means "per volume policy".
    # Applying the lastaccess transform there would turn 2 into 0 - which is why this is a
    # per-behavior switch and not one blanket mask.
    param(
        [AllowNull()][object]$Raw,
        [int[]]$ValidValues,
        [switch]$HighBitIsSystemManaged
    )

    if ($null -eq $Raw) { return $null }
    $numeric = [long]0
    if (-not [long]::TryParse(([string]$Raw).Trim(), [ref]$numeric)) { return $null }

    # 2147483648 in decimal, not 0x80000000: PowerShell parses a hex literal as the smallest
    # type that holds the bit pattern, so 0x80000000 becomes Int32 -2147483648 and widening it
    # for the -band produces 0xFFFFFFFF80000000 - which fails to match a positive 0x80000002
    # coming from fsutil text. The decimal literal is unambiguously Int64.
    if ($HighBitIsSystemManaged -and (($numeric -band 2147483648) -ne 0)) {
        $state = [int]($numeric -band 0x1)
        $state = $state -bor 0x2
        if ($ValidValues -contains $state) { return $state }
        return $null
    }

    # An exact match always wins, so a plain in-range value is never reinterpreted.
    if ($numeric -ge 0 -and $numeric -le [int]::MaxValue -and ($ValidValues -contains [int]$numeric)) {
        return [int]$numeric
    }

    $masked = [int]($numeric -band 0x3)
    if ($ValidValues -contains $masked) { return $masked }
    return $null
}
...
function Get-RaseFsutilNumericValue {
    ...
    $info = $Global:RaseFsBehaviorInfo[$Behavior]
    $regName = $null
    $anchors = @()
    $highBit = $false
    if ($info -is [hashtable]) {
        if ($info.ContainsKey("RegistryName")) { $regName = [string]$info["RegistryName"] }
        if ($info.ContainsKey("Anchors")) { $anchors = @($info["Anchors"]) }
        if ($info.ContainsKey("HighBitIsSystemManaged")) { $highBit = [bool]$info["HighBitIsSystemManaged"] }
    }

    # 1) Registry (authoritative, locale-independent, and the ONLY source that carries the
    #    system-managed flag - fsutil prints that part as prose, never as part of the number).
    if ($regName) {
        try {
            $key = Get-ItemProperty -Path $Global:RaseFileSystemRegKey -Name $regName -ErrorAction Stop
            $resolved = ConvertTo-RaseFsBehaviorValue -Raw $key.$regName -ValidValues $ValidValues -HighBitIsSystemManaged:$highBit
    ...
    # 2) fsutil text fallback, anchored on English tokens rather than on separators or prose.
    #
    # $ErrorActionPreference is dropped to Continue around the call on purpose. In Windows
    # PowerShell 5.1, redirecting a native command's stderr with 2>&1 while the preference is
    # Stop turns any stderr output into a terminating NativeCommandError - so a tool that merely
    # writes a note to stderr aborts the whole function before it can be parsed, or even
    # recorded. That is the most likely reason the v73.6.5 diagnostic never printed.
    $previousEap = $ErrorActionPreference
    try {
        $ErrorActionPreference = 'Continue'
        $output = @(& fsutil.exe behavior query $Behavior 2>&1 | ForEach-Object { [string]$_ }) -join "`n"
        $exitCode = $LASTEXITCODE
        $ErrorActionPreference = $previousEap

        $Global:RaseLastFsutilOutput = $output
        if ($exitCode -ne 0 -or [string]::IsNullOrWhiteSpace($output)) { return $null }

        foreach ($anchor in $anchors) {
            if ([string]::IsNullOrWhiteSpace($anchor)) { continue }
            $anchored = [regex]::Match($output, ('(?is)' + [regex]::Escape($anchor) + '\D{0,60}?(\d+)'))
            if ($anchored.Success) {
                $resolved = ConvertTo-RaseFsBehaviorValue -Raw $anchored.Groups[1].Value -ValidValues $ValidValues
                if ($null -ne $resolved) { return $resolved }
            }
        }

        # 3) Separator-anchored parse, restored after the probe showed disable8dot3 carries no
        #    English token whatsoever on a localized build. This is deliberately NOT a scan for
        #    "any digit": the output for 8.3 contains an 8 and a 3 inside the word "8dot3", and
        #    a naive scan would happily return one of those. It matches only a number that
        #    directly follows '=' or ':' - punctuation is not translated - and takes the FIRST
        #    such number, because the setting value always precedes the descriptive text.
        $separatorMatch = [regex]::Match($output, '(?m)(?:=|:)\s*(\d+)')
        if ($separatorMatch.Success) {
            $resolved = ConvertTo-RaseFsBehaviorValue -Raw $separatorMatch.Groups[1].Value -ValidValues $ValidValues
            if ($null -ne $resolved) { return $resolved }
        }
        ...
        $Global:RaseLastFsutilOutput = "fsutil query raised: " + $_.Exception.Message
...
function Invoke-NtfsTuning {
    ...
    try {
        ...
        if ($null -eq $disableLastAccess) {
            ...
            # Show what fsutil actually returned or raised. Without this, diagnosing a detection
            # failure on someone else's build or locale means guessing from a distance.
            if (-not [string]::IsNullOrWhiteSpace([string]$Global:RaseLastFsutilOutput)) {
                $rawSample = ([string]$Global:RaseLastFsutilOutput) -replace '\s+', ' '
                if ($rawSample.Length -gt 300) { $rawSample = $rawSample.Substring(0, 300) + "..." }
                HtmlAdd ("fsutil returned: " + (ConvertTo-HtmlSafe $rawSample)) "#808080"
            } else {
                HtmlAdd "fsutil produced no capturable output for this query." "#808080"
            }
...
try {
    ...
    if (-not $Global:IsHeadless) {
        ...
        # Close the transcript BEFORE blocking on input. Stop-Transcript lives in the finally
        # block, which does not run until the operator answers - so while RASE waits, the
        # transcript file has no footer and is incomplete. Both Full acceptance-run transcripts
        # arrived truncated for exactly this reason, and a console closed with the X button
        # would leave the file that way permanently. The finally block's guard prevents a
        # double stop.
        if ($Global:TranscriptStarted) {
            try { Stop-Transcript | Out-Null; $Global:TranscriptStarted = $false } catch {}
        }
